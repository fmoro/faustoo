...
if [[ $(get_version_component_count) == 3 ]]; then
	...
	SRC_URI_FORMAT="http://%s.visual-paradigm.com/visual-paradigm/${PN}$(get_version_component_range 1-2)/$(get_version_component_range 3)/${MY_PN}_Linux_NoInstall_${MY_PV}.tar.gz http://%s.visual-paradigm.com/archives/${PN}$(get_version_component_range 1-2)/$(get_version_component_range 3)/${MY_PN}_Linux_NoInstall_${MY_PV}.tar.gz"
else
